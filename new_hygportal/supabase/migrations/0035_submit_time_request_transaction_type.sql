-- Allow ESARF submissions to preserve the selected transaction label while
-- keeping the existing submit_time_request RPC signature working.

drop function if exists public.submit_time_request(text, date, date, time, time, numeric, text, text, text, text);
