...
begin
  ...
  from unnest(p_employee_ids) as selected(selected_employee_id)
  where not exists (
    select 1
    from public.employee_assignments ea
    where ea.employee_id = selected.selected_employee_id
  ...
  select
    ...
    now()
  from unnest(p_employee_ids) as selected(selected_employee_id)
  on conflict on constraint manager_team_schedules_one_per_day
  do update set
  ...
  select
    ...
    schedules.employee_name,
    schedules.from_time,
    schedules.to_time,
    schedules.is_day_off,
    schedules.notes
  from public.manager_my_team_schedules() schedules;
end;
$$;
