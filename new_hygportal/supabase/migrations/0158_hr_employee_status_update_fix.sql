-- Migration 0158: Fix employee status updating in hr_update_employee_profile, hr_employee_profile_detail, and hr_set_employee_status.

-- 1. Update hr_update_employee_profile to accept p_employment_status and update public.employees.employment_status.
drop function if exists public.hr_update_employee_profile(
  text, text, uuid, text, text, text, text, date, text, text, text, text, text, text, text, date, text, text, text, text, text, text, text, text, text, text, text, text
);
drop function if exists public.hr_update_employee_profile(
  text, text, uuid, text, text, text, text, date, text, text, text, text, text, text, text, date, text, text, text, text, text, text, text, text, text, text, text, text, text
);
...
declare
  ...
  v_status text := nullif(lower(trim(coalesce(p_employment_status, ''))), '');
begin
  if not (
    public.is_hr_staff()
    or (
  ...
  set
    ...
    employment_status = coalesce(v_status, employment_status),
    updated_at = now()
  ...
  values (
    ...
    nullif(trim(coalesce(p_time_schedule, '')), ''),
    nullif(trim(coalesce(p_day_off_day, '')), ''),
...
-- 2. Ensure hr_set_employee_status function is properly defined and granted.
...
-- 3. Update hr_employee_profile_detail to include employment_status column in output.
...
-- 4. Bulk update existing employee records whose status was left as 'pending' or null in public.employees table to 'active'
update public.employees
set employment_status = 'active', updated_at = now()
where lower(trim(coalesce(employment_status, ''))) in ('pending', '');
