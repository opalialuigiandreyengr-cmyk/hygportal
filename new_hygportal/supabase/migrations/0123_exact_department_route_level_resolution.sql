-- Keep explicitly configured department approval route levels exact.
-- If a route says Level 7, a missing Level 7 approver must not be replaced by Level 8.
...
with affected_steps as (
  ...
  where d.name = 'IT'
    and rt.code in ('overtime', 'offset_earn', 'use_offset')
    and ras.required_level <> route.approver_level
    and route.approver_level = 7
    and ras.required_level = 8
...
resolved as (
  select
    ...
    approver.resolved_level
  from affected_steps
  left join lateral public.find_request_approver(
...
set required_level = resolved.expected_level,
    assigned_approver_employee_id = resolved.approver_employee_id,
    assigned_approver_user_id = resolved.approver_user_profile_id,
    status = case
      when resolved.approver_employee_id is null and ras.status = 'pending' then 'admin_fallback'
      else ras.status
    end,
    skipped_reason = case
      when resolved.approver_employee_id is null then coalesce(ras.skipped_reason, 'No matching approver found.')
      else null
    end
from resolved
