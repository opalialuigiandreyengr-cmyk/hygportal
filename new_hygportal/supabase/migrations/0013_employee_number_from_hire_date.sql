  create or replace function public.next_employee_no_for_hire_date(p_date_hired date)
  returns text
  language plpgsql
  security definer
  ...
  begin
    if p_date_hired is null then
      raise exception 'Date hired is required to generate an employee number.';
    end if;

    v_prefix := to_char(p_date_hired, 'MMDDYYYY');

    select coalesce(max(nullif(split_part(employee_no, '-', 2), '')::int), 0) + 1
    into v_next_no
    from public.employees
    where employee_no ~ ('^' || v_prefix || '-[0-9]+$');

    return v_prefix || '-' || lpad(v_next_no::text, 2, '0');
  end;
  $$;

  grant execute on function public.next_employee_no_for_hire_date(date) to anon, authenticated;
  ...
  begin
    ...
    loop
      v_employee_no := public.next_employee_no_for_hire_date(p_date_hired);
