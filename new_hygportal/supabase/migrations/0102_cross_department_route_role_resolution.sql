-- Treat an explicitly selected approval-route role as a cross-department
-- override. If IT tags an Operations role under Department + Position and
-- chooses that role in Approval Routes, the resolver should find employees in
-- that role even when their own assignment/function belongs to Operations.
...
begin
  ...
  for v_level in v_search_start_level..8 loop
    ...
    loop
      if v_required_position_id is not null
        or v_level >= 7
