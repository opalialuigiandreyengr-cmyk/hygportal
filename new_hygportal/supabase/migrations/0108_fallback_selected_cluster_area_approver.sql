-- If a selected Cluster/Area Manager role has no exact scoped authority row yet,
-- fall back to the selected role itself so the route still auto-resolves.
...
declare
  ...
  v_fallback_employee_id uuid;
  v_fallback_level int;
begin
  select *
  into v_assignment
  ...
  for v_level in v_search_start_level..8 loop
    v_fallback_employee_id := null;
    v_fallback_level := null;
    ...
    loop
      ...
      if (
        v_required_position_id is not null
        and v_level = v_search_start_level
        and v_level in (4, 5)
        and v_fallback_employee_id is null
      ) then
        v_fallback_employee_id := v_authority.employee_id;
        v_fallback_level := v_level;
      end if;
    ...
    if v_fallback_employee_id is not null then
      approver_employee_id := v_fallback_employee_id;
      select up.id
      into approver_user_profile_id
      from public.user_profiles up
      where up.employee_id = v_fallback_employee_id
      limit 1;
      resolved_level := v_fallback_level;
      return next;
      return;
    end if;
  end loop;
