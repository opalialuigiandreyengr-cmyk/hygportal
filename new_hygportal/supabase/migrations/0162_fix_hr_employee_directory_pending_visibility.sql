-- Migration 0162: Fix hr_employee_directory birth_date error and strict HR company isolation.
...
begin
  ...
  select
    ...
    e.phone,
    e.photo_url,
    coalesce(nullif(trim(e.employment_status), ''), 'pending') as employment_status,
    e.birth_date,
  ...
  where (
    ...
    or exists (
      select 1
      from public.employee_assignments ea_check
      left join public.stores s_check on s_check.id = ea_check.store_id
      where ea_check.employee_id = e.id
        and (
          ea_check.company_id in (
            select company_id
            from public.hr_company_assignments
            where user_profile_id = v_user_profile_id
          )
          or s_check.company_id in (
            select company_id
            from public.hr_company_assignments
            where user_profile_id = v_user_profile_id
          )
