-- Make Flutter cluster maintenance store-driven while keeping company inferred.
...
begin
  ...
  select count(*), count(distinct s.company_id), min(s.company_id)
  into v_valid_store_count, v_company_count, v_company_id
  ...
  if v_company_count <> 1 then
    raise exception 'Selected stores must belong to one company.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Cluster already exists for these stores.';
  end if;
