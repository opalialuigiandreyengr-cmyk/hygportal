-- Allow HR users to sign in to the desktop portal without granting admin powers.
...
begin
  ...
  if v_auth_user.id is null then
    raise exception 'Desktop login is required.';
  end if;
  ...
  if v_profile.id is null or v_profile.app_role not in ('hr', 'admin', 'super_admin') then
    raise exception 'HR desktop access is required.';
  end if;
