-- Include Reason of Inactivity and Date Inactive in the HR desktop supplemental profile save/load path.

alter table if exists public.employee_profile_details
add column if not exists reason_of_inactivity text,
add column if not exists date_inactive text;
...
begin
  ...
  values (
    ...
    nullif(trim(p_profile->>'reasonOfInactivity'), ''),
    nullif(trim(p_profile->>'dateInactive'), ''),
  ...
  set
    ...
    reason_of_inactivity = excluded.reason_of_inactivity,
