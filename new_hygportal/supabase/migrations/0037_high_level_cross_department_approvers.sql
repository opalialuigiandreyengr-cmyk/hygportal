-- Let senior approver roles approve across departments within the same company.
-- Department routes still decide the required levels. This only changes the
-- employee resolver so L7/L8 leaders are not blocked by requester function_id.
