-- Notify approvers for ESARF/leave assignments and requesters for final outcomes.

alter table public.approval_push_outbox
  alter column approval_step_id drop not null;

alter table public.approval_push_outbox
  drop constraint if exists approval_push_outbox_approval_step_id_key;

alter table public.approval_push_outbox
  add column if not exists notification_key text;

update public.approval_push_outbox
set notification_key = 'approval:' || approval_step_id::text
where notification_key is null;

alter table public.approval_push_outbox
  alter column notification_key set not null;

create unique index if not exists idx_approval_push_outbox_notification_key
  on public.approval_push_outbox(notification_key);
...
drop trigger if exists trg_enqueue_request_outcome_notification on public.requests;
create trigger trg_enqueue_request_outcome_notification
after update of status on public.requests
for each row
execute function public.enqueue_request_outcome_notification();
